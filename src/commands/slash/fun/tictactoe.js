      ...
      collector.on('collect', async button => {
        if (button.customId == 'declinettt') {
          ...
          let msg = await interaction.channel.send({
            content: `等待 | <@!${Args.userid}>, 您的符號： ${o_emoji}`
          })
          tictactoe(msg)
          ...
          async function tictactoe (m) {
            ...
            if (won['<:O_:863314110560993340>'] != false) {
              if (Args.user == 0)
                return m.edit({
                  content: `<@!${fighters[1]}> (${o_emoji}) 贏了，真是場精彩的遊戲。`,
                  components: []
                })
              else if (Args.user == 1)
                return m.edit({
                  content: `<@!${fighters[0]}> (${o_emoji}) 贏了，真是場精彩的遊戲。`,
                  components: []
                })
            }
            if (
            ...
            if (won['<:X_:863314044781723668>'] != false) {
              if (Args.user == 0)
                return m.edit({
                  content: `<@!${fighters[1]}> (${x_emoji}) 贏了，真是場精彩的遊戲。`,
                  components: []
                })
              else if (Args.user == 1)
                return m.edit({
                  content: `<@!${fighters[0]}> (${x_emoji}) 贏了，真是場精彩的遊戲。`,
                  components: []
                })
            }
            ...
            collector.on('collect', b => {
              if (b.user.id !== Args.userid)
                return b.reply({ content: '等待你的回合。', ephemeral: true })
              ...
              if (Object.keys(Brgs).length == 0)
                return m.edit({ content: '平手！', components: [] })
              tictactoe(m)
            })
            collector.on('end', collected => {
              if (collected.size == 0)
                m.edit({
                  content: `<@!${Args.userid}> 沒有在時間內做出回應。`,
                  components: []
                })
            })
          }
