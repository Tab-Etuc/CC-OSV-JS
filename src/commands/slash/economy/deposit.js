  ...
  options: [
    {
      ...
      type: 'STRING',
      description:
        '您欲存入的現金金額。（註：輸入"all"、"所有"等，可一次性存入所有現金。）',
      required: true
    }
  ],
    ...
    if (
      ...
      if (data.coinsInWallet > data.bankSpace) {
        ...
        await interaction.editReply(
          `已存入 \`${data.bankSpace - data.coinsInBank}\` 元！`
        )
        ...
        if (data.coinsInWallet + data.coinsInBank > data.bankSpace) {
          ...
          interaction.editReply(
            `已存入 \`${(data.coinsInWallet - left).toLocaleString()}\` 元！`
          )
          ...
          interaction.editReply(
            `已存入 \`${data.coinsInWallet.toLocaleString()}\` 元！`
          )
      ...
      if (isNaN(arg)) {
        return interaction.editReply('請您輸入一個正確的數字。')
      }

      if (parseInt(arg) > data.bankSpace) {
        return interaction.editReply('您的銀行存額不足，請提升。')
      }
      if (parseInt(arg) > data.coinsInWallet) {
        return interaction.editReply('您沒有這麼多錢。')
      }
      ...
      await interaction.editReply(`已存入 \`${args[0]}\` 元！`)
