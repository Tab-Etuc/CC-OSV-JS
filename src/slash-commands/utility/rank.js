  ...
  async execute (bot, interaction) {
    ...
    rank =
      rank
        ...
        .findIndex(x => x.userId == member.id) + 1
    ...
    const background = await Canvas.loadImage(
      'https://cdn.discordapp.com/attachments/854215565882294274/876791656605708358/image_7.png'
    )
    ...
    context.font = applyText_eng(
      canvas,
      `${bot.utils.numberFormat(
        levelData ? levelData.xp : 0
      )}/${bot.utils.numberFormat(levelData ? levelData.xpToLevel : 100)}`
    )
    context.fillStyle = '#ffffff'
    context.fillText(
      `${bot.utils.numberFormat(
    ...
    let lingrad = context.createLinearGradient(
      canvas.width / 2.8,
      canvas.height / 1.35,
      295,
      25
    ...
    context.font =
      0 <= levelP && levelP <= 9
        ? `15px Gen Jyuu GothicX Medium`
        : `20px Gen Jyuu GothicX Medium`
