...
},
{
    ...
    run: async (bot, message, args) => {
        ...
        if (findItem) {
            userInv.push({ name: 'gem', amount: (findItem.amount + gemAmount), description: '販賣寶石以賺取利潤。' });
            data.items = userInv;
            await data.save();
        } else {
            userInv.push({ name: 'gem', amount: gemAmount, description: '販賣寶石以賺取利潤。' });
            data.items = userInv;
            await data.save();
        }
    }
},
{
    ...
    run: async (bot, message, args) => {
        ...
        const data = await bot.fetchUser(message.author.id);
        message.channel.send(`您走進了樹林並把其中**${treeAmount}**棵倒楣的樹🌲砍了下來。`);
        const findItem = data.items.find(i => i.name.toLowerCase() == '木材');
        let userInv = data.items.filter(i => i.name.toLowerCase() !== '木材');
        if (findItem) {
            userInv.push({ name: '木材', amount: (findItem.amount + treeAmount), description: '販賣木材以賺取利潤。' });
            data.items = userInv;
            await data.save();
        } else {
            userInv.push({ name: '木材', amount: treeAmount, description: '販賣木材以賺取利潤。' });
            data.items = userInv;
            await data.save();
        }
    }
