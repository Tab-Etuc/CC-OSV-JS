...
class Time {
  ...
  getHour() {
    return this.t.toISOString().replace(/T|:/g, "-").split("-")[3];
  }

  getDate() {
    return this.t.toISOString().replace(/T|:/g, "-").split("-");
  }
...
function parseDateNow() {
  let getDifferenceToUtcInMilisec = new Date().getTimezoneOffset() *
    MINUTES_TO_MILISECOND;
