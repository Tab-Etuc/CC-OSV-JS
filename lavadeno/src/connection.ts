...
export class Connection<N extends Node = Node> {
  ...
  async connect() {
    ...
    try {
      ...
      this.#_socket = await pogsockets.connectPogSocket(
        `ws${this.info.secure ? "s" : ""}://${this.address}`,
        { headers },
      );
    } catch (e) {
  ...
  disconnect(code = 1000, reason = "disconnecting...") {
    ...
    this.node.debug(
      "connection",
      `disconnecting... code=${code}, reason=${reason}`,
    );
    ...
    for await (const event of pogsockets.readSocket(this.#_socket!)) {
      ...
      if (event.type === "pong") {
        ...
        return this.node.debug(
          "connection",
          `received pong event${
            this.latency ? `, latency=${this.latency}ms` : "."
          }`,
        );
      }
  ...
  private _onmessage({ data }: pogsockets.MessageEvent) {
    ...
    this.node.debug(
      "connection",
      `${constants.clientName} <<< ${payload.op} | ${data}`,
    );
  ...
  private async _onclose(event: pogsockets.CloseEvent) {
    ...
    const reconnecting = !!this.reconnectOptions &&
  ...
  private _send(payload: Lavalink.OutgoingMessage): Promise<void> {
    ...
    this.node.debug(
      "connection",
      `${constants.clientName} >>> ${payload.op} | ${json}`,
    );
